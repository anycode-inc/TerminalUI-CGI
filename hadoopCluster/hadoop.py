...
def copyTemplate(nodeIP):
    subprocess.run(f'scp ./hadoopCluster/temp/hdfs-site.xml root@{nodeIP}:/root/hdfs-site.xml',shell=True)
    subprocess.run(f'scp ./hadoopCluster/temp/core-site.xml root@{nodeIP}:/root/core-site.xml',shell=True)
...
def nameNode(nameNodeIP):
    ...
    subprocess.run(f"ssh root@{nameNodeIP} 'bash -s' < ./hadoopCluster/temp/installationScript.sh",shell=True)
    return nameNodePort
...
def dataNode(dataNodeIP,nameNodeIP,nameNodePort):
    ...
    subprocess.run(f"ssh root@{dataNodeIP} 'bash -s' < ./hadoopCluster/temp/installationScript.sh",shell=True)
